...
use super::uri::{derive_connect_address, get_server_api_address, get_server_root_http_address, resolve_maybe_relative_url};
...
pub fn download_client_for_server(
    paths: &LauncherPaths,
    server_address: &str,
    info: &ServerInfo,
) -> Result<ClientInstall> {
    download_client_for_server_with_proxy(paths, server_address, info, None)
}

pub fn download_client_for_server_with_proxy(
    paths: &LauncherPaths,
    server_address: &str,
    info: &ServerInfo,
    proxy_url: Option<&str>,
) -> Result<ClientInstall> {
    download_client_for_server_with_proxy_and_auth(paths, server_address, info, proxy_url, None)
}

pub fn download_client_for_server_with_proxy_and_auth(
    paths: &LauncherPaths,
    server_address: &str,
    info: &ServerInfo,
    proxy_url: Option<&str>,
    ...
    let tokens: Vec<Option<&str>> = vec![auth_token];
...
pub fn download_client_for_server_with_proxy_and_tokens(
    ...
    let download_urls = candidate_download_urls(server_address, build)?;
    ...
    let mut hard_failed: HashSet<String> = HashSet::new();
    ...
    for token in auth_tokens {
        ...
        if trimmed.is_empty() {
            seen_tokens.insert(String::new());
            tokens.push(None);
            continue;
        }
        if seen_tokens.insert(trimmed.clone()) {
            tokens.push(Some(trimmed));
        }
    }
    if tokens.is_empty() {
        tokens.push(None);
    ...
    'url_loop: for download_url in &download_urls {
        if hard_failed.contains(download_url.as_str()) {
            continue;
        }

        let mut auth_issue_for_url = false;

        for token in &tokens {
            let auth_modes: Vec<(&str, Option<String>)> = if let Some(token) = token {
                vec![
                    ("SS14Auth", Some(format!("SS14Auth {token}"))),
            ...
            for (auth_label, auth_header) in auth_modes {
                ...
                attempts.push(format!("{} [{}] -> {}", download_url, auth_label, status));

                if !status.is_success() {
                    last_status = Some(status);

                    if status == StatusCode::UNAUTHORIZED || status == StatusCode::FORBIDDEN {
                        auth_issue_for_url = true;
                        break;
                    }

                    hard_failed.insert(download_url.clone());
                    break;
                }

                let mut response = response;
                let mut out_file =
                    fs::File::create(&zip_path).with_context(|| format!("creating {}", display_path(&zip_path)))?;
        ...
        let _ = auth_issue_for_url;
    }

    if !downloaded {
        let attempt_report = if attempts.is_empty() {
            String::from("<no endpoints attempted>")
        } else {
            attempts.join("; ")
        };
        ...
        match last_status {
            Some(status) => anyhow::bail!(
                "client zip endpoint responded with {status}; attempts: {attempt_report}"
            ),
            None => anyhow::bail!(
                "client zip download failed before receiving any response; attempts: {attempt_report}"
            ),
        }
    }
...
fn candidate_download_urls(server_address: &str, build: &ServerBuildInformation) -> Result<Vec<String>> {
    let mut urls = Vec::new();
    let api_base = get_server_api_address(server_address)?;
    let root_base = get_server_root_http_address(server_address)?;

    let should_infer_selfhosted = build.acz.unwrap_or(false)
        || build
            .download_url
            .as_deref()
    ...
    if let Some(url) = &build.download_url {
        if !url.trim().is_empty() {
            let resolved = resolve_maybe_relative_url(&api_base, url.trim())?;
            urls.push(resolved.clone());
            urls.extend(nearby_filename_candidates(&resolved)?);
        }
    }

    if should_infer_selfhosted {
        urls.push(format!("{api_base}client.zip"));
    }

    urls.push(format!("{api_base}download"));
    urls.push(format!("{api_base}client.zip"));
    urls.push(format!("{api_base}Content.Client.zip"));
    urls.push(format!("{api_base}SS14.Client.zip"));
    urls.push(format!("{root_base}download"));
    urls.push(format!("de{root_base}client.zip"));
    urls.push(format!("{root_base}Content.Client.zip"));
    urls.push(format!("{root_base}SS14.Client.zip"));
...
fn nearby_filename_candidates(explicit_url: &str) -> Result<Vec<String>> {
    let parsed = reqwest::Url::parse(explicit_url).context("invalid explicit download URL")?;
    let mut out = Vec::new();
    ...
    if let Some(segments) = parsed.path_segments() {
        ...
        let mut push_with_name = |name: &str| {
        ...
        push_with_name("Content.Client.zip");
        push_with_name("SS14.Client.zip");
        push_with_name("client.zip");
    }
